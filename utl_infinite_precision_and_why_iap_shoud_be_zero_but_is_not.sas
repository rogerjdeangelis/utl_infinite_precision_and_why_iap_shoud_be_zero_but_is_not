...
128bit floats would be nice?
  ...
  Three SOLUTIONS
    ...
    1. Reduces the error by about and order of magnitude
...
SELECT DISTINCT
       ...
       (CASE WHEN c  = 'WO' THEN -1 ELSE 1 END) * d AS PRVN_A
        ,SUM (CALCULATED PRVN_A) AS iap format=22.12
FROM T
GROUP BY 1
ORDER BY a ;
QUIT;
...
--------------------------------------------------------------------------------
00000000481710428888  07Mar201  IP        175455.8  175455.8   0.000000000007276
...
SELECT DISTINCT
       ...
       d,
       (CASE WHEN c  = 'WO' THEN -1 ELSE 1 END) * d AS PRVN_A
       ,SUM (CALCULATED PRVN_A)/100 AS iap format=22.12
FROM T
GROUP BY 1
ORDER BY a ;
QUIT;
